...
def read_disk(request):
    ...
    df = pd.DataFrame(columns=['inicio'])
    for line in command:
        df = df.append({'inicio': line}, ignore_index=True)
    df = df[df.inicio != ".\n" ]
    df["inicio"] = df["inicio"].str.replace('[',',')
    df["inicio"] = df["inicio"].str.replace(']',',')
    df["inicio"] = df["inicio"].str.replace(' ',',')
    df["inicio"] = df["inicio"].str.replace(',,,,,,,',',')
    df["inicio"] = df["inicio"].str.replace(',,',',')
    df["inicio"] = df["inicio"].str.replace(',,,',',')
    df["inicio"] = df["inicio"].str.replace(',,',',')
    df2 = pd.DataFrame(df.inicio.str.split('\n',1).tolist(), columns = ['other', "other2"])
    df2 = pd.DataFrame(df2["other"])
    df2['other'].replace('', np.nan, inplace=True)
    df2= pd.DataFrame(df2.other.dropna()).astype(str)
    df2 = pd.DataFrame(df2.other.str.split(',',5).tolist(), columns=["xxx","Tamaño","Mes","Dia","Año", "Ruta"])
    df2 = df2.drop('xxx', 1)
    df2["Mes"] = df2["Mes"].str.replace('Jan','1')
    df2["Mes"] = df2["Mes"].str.replace('Feb','2')
    df2["Mes"] = df2["Mes"].str.replace('Marc','3')
    df2["Mes"] = df2["Mes"].str.replace('Apr','4')
    df2["Mes"] = df2["Mes"].str.replace('May','5')
    df2["Mes"] = df2["Mes"].str.replace('Jun','6')
    df2["Mes"] = df2["Mes"].str.replace('Jul','7')
    df2["Mes"] = df2["Mes"].str.replace('Aug','8')
    df2["Mes"] = df2["Mes"].str.replace('Sep','9')
    df2["Mes"] = df2["Mes"].str.replace('Oct','10')
    df2["Mes"] = df2["Mes"].str.replace('Nov','11')
    df2["Mes"] = df2["Mes"].str.replace('Dec','12')

    mylist = zip(df2["Tamaño"], df2["Mes"], df2["Dia"], df2["Año"], df2["Ruta"])
